...
@export var MOTOR_VELOCITY := 200.0 # not sure of the units
...
func _physics_process(delta: float) -> void:
	if true or owner == null: # scene being tested standalone
		test_input(delta)
	pass
...
func current_arm_position() -> ArmPosition:
	...
	easy_print_v3(qq)
	var pos: Vector3 = Vector3.ZERO - basis * $ConeJoint.position
	...
	#print(str(round(thetaro.theta*10)/10.) + " " + str(round(thetaro.ro*10)/10.))
	return thetaro
